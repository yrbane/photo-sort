...
fn date_from_exif(path: &Path) -> Option<NaiveDateTime> {
...
fn date_from_dirname(path: &Path) -> Option<NaiveDateTime> {
    let re = Regex::new(r"(19|20)\d{2}").unwrap();
    let path_str = path.to_string_lossy();
    // Search from right to left by collecting all matches and taking the last one,
    // which is closest to the file in the directory hierarchy.
    let year: u32 = re
        .find_iter(&path_str)
        .last()?
        .as_str()
...
fn date_from_filesystem(path: &Path) -> Option<NaiveDateTime> {
...
fn detect_date(path: &Path) -> (NaiveDateTime, DateSource) {
...
fn build_dest_path(output_dir: &Path, dt: &NaiveDateTime, ext: &str) -> PathBuf {
...
fn save_progress(path: &Path, progress: &Progress) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    let source = cli
        .source
        .canonicalize()
        .with_context(|| format!("Dossier source introuvable : {}", cli.source.display()))?;
    ...
    // Index processed files for fast lookup
    ...
    for photo_path in &photos {
        ...
        // Skip if already processed with same size
    ...
    if !years_created.is_empty() {
        ...
        println!(
            "  {}  {}",
            style("Années").dim(),
            style(years.iter().map(|y| y.as_str()).collect::<Vec<_>>().join(", ")).cyan()
        );
    }
    println!();
...
mod tests {
    ...
    fn is_photo_recognizes_supported_extensions() {
        for ext in PHOTO_EXTENSIONS {
            assert!(is_photo(Path::new(&format!("photo.{ext}"))), "devrait reconnaître .{ext}");
        }
    }
    ...
    fn dirname_takes_last_year_match() {
        // The deepest directory component should win
    ...
    fn dirname_rejects_out_of_range_years() {
        // 1899 and 2100 are outside (19|20)\d{2}
    ...
    fn dest_path_collision_increments() {
        ...
        // Create the base file to force collision
    ...
    fn detect_date_uses_dirname_for_non_exif_file() {
        ...
        // Should pick up 2015 from dirname (file has no EXIF)
    ...
    fn detect_date_falls_back_to_filesystem() {
        ...
        // No EXIF, no year in path → filesystem
